    ...
    public class ComponentTitlescreenHooks : MonoBehaviour
    {
        ...
        public void HostGame()
        {
            ...
            if(Unity.Netcode.NetworkManager.Singleton.StartServer())
            {
                ...
                Unity.Netcode.NetworkManager.Singleton.SceneManager.LoadScene(Constants.GAME_SCENE_NAME, UnityEngine.SceneManagement.LoadSceneMode.Single);
            }
            else
            {
                EnableEverything();
        ...
        private IEnumerator ConnectTimeout()
        {
            ...
            if(SceneManager.GetActiveScene().buildIndex == Constants.TITLESCREEN_BUILD_INDEX && NetworkManager.Singleton != null)
            {
                EnableEverything();
                TimeoutText.gameObject.SetActive(true);
                StartCoroutine(Countdown5Seconds());
